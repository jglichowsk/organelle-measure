...
path_in=r'C:\Users\jglic\Downloads\2-25-26 3c-hap-selected\BF-timelapse_sum1-4_02252026_3c-hap_fov2.tif'
img_in=io.imread(str(path_in)) #load in bright-field time series.

#optimized affine transform params from fluorescent bead reference images.
# params=[.9905,  .9912,  19.52, -28.11, 87.82, 6.619e-03, -5.874e-03] #1024 -> 1024 params [110 nm laser px]
# params=[1.007, 1.008, 6.174, -22.54, 87.79, 6.487e-03, -5.678e-03] #1024 -> 512 params [216 nm laser px]
# params=[ 1.049, 1.048,-6.500,-20.00, 87.82,1.000e-02,-2.079e-03] #[400 nm laser px]
...
if len(img_in.shape)==2: #for time-snapshot (2D).
...
elif len(img_in.shape)==3: #for single-z time lapse (3D); txy or tyx
...
## Currently have to manually turn stack into hyperstack in ImageJ
elif len(img_in.shape)==4: #for z-stack time lapse (4D); tzxy or tzyx
    stack_out=[]
    for j in range(img_in.shape[1]): #for each z slice
        warped_frame=[] #initalize empty list to append processed slices to.
        for i in range(img_in.shape[0]): #for each time point in series
            img_dummy=img_in[i,j,:,:] #create dummy array to perform operations and maintain changes. 
            img_downsc=transform.rescale(img_dummy,0.5, anti_aliasing=True)
            img_out = np.zeros((512,512),dtype=float) # hard coded size, okay.
            shape0,shape1 = img_downsc.shape
            img_out[:shape0,:shape1] = img_downsc #account for dimension cutoff in full camera fov. Also normalize image here.
            camera_rot=transform.rotate(img_out,theta) #rotate the raw image.
            dummy_warped=transform.warp(camera_rot,tform.inverse) #apply the affine transform to the rotated camera image.
            warped_frame.append(dummy_warped/np.max(dummy_warped))
        warped_frame=np.asarray(warped_frame) #turn nested list into 3D array.
        stack_out.append(warped_frame)
    camera_warped=np.stack(stack_out, axis=1) #stack slices with z being second axis
