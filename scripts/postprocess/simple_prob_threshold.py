...
def apply_thresh(img_path):
    org_prob=io.imread(img_path) #read in pixel probability image
    bkgd_prob=1-org_prob
    org_mask=(org_prob>bkgd_prob) #intensity counted as true signal when greater than threshold.
    io.imsave(str(img_path)[:-18]+'mask.tiff', util.img_as_uint(org_mask))
